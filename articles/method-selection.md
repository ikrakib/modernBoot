...
library(modernBoot) set.seed(42)

## Introduction to modernBoot

The **modernBoot** package provides modern resampling methods for
statistical inference beyond standard parametric assumptions. This
...
when: - Your data are independent and identically distributed (IID) -
Sample size is moderate to large (n ≥ 30) - You want confidence
intervals for the mean

**Example: Bootstrap CI for mean height** {r bootstrap-example} \#
Simulate height measurements (in cm) heights \<- c(172, 168, 175, 170,
180, 165, 178, 172, 169, 176)

## Percentile bootstrap CI

result_bs \<- bs_mean(heights, R = 1000, conf = 0.95) cat(“Sample
mean:”, round(result_bs\$stat, 2), "\n") cat("95% CI:",
round(result_bs\$ci, 2), “to”, round(result_bs\$ci, 2), “”)

## BCa bootstrap (bias-corrected)

result_bca \<- bca_ci(heights, R = 1000, conf = 0.95) cat(“BCa CI lower
bound:”, round(result_bca\$ci, 2), "\n") cat("BCa CI upper bound:",
round(result_bca\$ci, 2), “”)
...
when: - Your data have temporal dependence (autocorrelation) - Standard
IID bootstrap would be invalid - Examples: stock prices, climate data,
economic time series

**Example: Moving block bootstrap for AR(1) process** {r
timeseries-example} \# Generate AR(1) time series ts_data \<-
arima.sim(n = 100, list(ar = 0.7), sd = 1)
...
cat(“Original mean:”, round(mean(ts_data), 3), “”) cat(“Bootstrap mean
(average):”, round(mean(bootstrap_means), 3), “”) cat(“Bootstrap SE:”,
round(sd(bootstrap_means), 3), “”)
...
when: - You’re fitting linear regression with **heteroscedasticity** -
Residuals have non-constant variance - You want robust inference for
coefficients

**Example: Wild bootstrap for heteroscedastic regression** {r
wild-boot-example} \# Generate heteroscedastic data set.seed(42) x \<-
seq(-2, 2, length = 50) y \<- 2 + 1.5 \* x + rnorm(50, sd = abs(x) \*
0.5) \# Variance depends on x
...
estimate:", round(wb_result\$coef, 3), “”)
...
**Weight schemes:** - **Rademacher**: ±1 with equal probability (faster,
robust) - **Mammen**: Empirically calibrated distribution
...
when: - Comparing two independent groups - Avoiding parametric
assumptions - Data are small or non-normal

**Example: Two-sample permutation test** {r perm-test-example} \# Group
A: untreated group_a \<- c(2.3, 4.1, 3.7, 2.9, 4.5, 3.2)
...
cat(“Mean Group A:”, round(mean(group_a), 2), “”) cat(“Mean Group B:”,
round(mean(group_b), 2), “”) cat(“Difference:”, round(perm_result\$obs,
2), "\n") cat("Two-sided p-value:", round(perm_result\$p.value, 4), “”)

**Interpretation:** If p-value \< 0.05, evidence of significant group
difference.
...
when: - Testing multiple variables/features simultaneously - Need to
control family-wise error rate (FWER) - Examples: genomics, fMRI, survey
data

**Example: Permutation maxT for multiple testing** {r perm-maxT-example}
\# Simulate gene expression data: 30 genes, 2 groups set.seed(42) data
\<- matrix(rnorm(600), nrow = 50, ncol = 12) \# 50 obs, 12 variables
...
print(round(maxT_result\$p.values\[1:5\], 4)) cat(“Variable 3 p-value:”,
round(maxT_result\$p.values, 4), “”)

**Key advantage:** Maintains FWER control while being more powerful than
Bonferroni.
...
{r auto-select} \# Example 1: IID data x_iid \<- rnorm(50) rec1 \<-
auto_select_method(x_iid) cat(“IID data recommendation:”, rec1\$method,
“”)

## Example 2: Time series

ts \<- arima.sim(n = 100, list(ar = 0.8)) rec2 \<-
auto_select_method(ts) cat(“Time series recommendation:”, rec2\$method,
“”)

## Example 3: Multivariate data

mat \<- matrix(rnorm(300), nrow = 50, ncol = 6) rec3 \<-
auto_select_method(mat) cat(“Multivariate recommendation:”,
rec3\$method, “”)
...
For extreme quantiles or skewed distributions, use
**[`studentized_ci()`](https://github.com/ikrakib/modernBoot/reference/studentized_ci.md)**:

{r studentized-example} \# Right-skewed data (exponential) data_skewed
\<- rexp(100, rate = 0.5)
...
ci_studentized \<- studentized_ci(data_skewed, q = 0.5, R = 500, Rinner
= 100)

cat(“Studentized 95% CI for median:”) cat(“Lower:”,
...
{r sim-example, eval = FALSE} \# Define data generator generator \<-
...
sim_results \<- compare_methods_sim( data_generator = generator, Rsim =
50, \# 50 simulated datasets Rboot = 1000, \# 1000 bootstrap replicates
each parallel = TRUE \# Use parallelization )

## Analyze coverage

coverage_bs \<- mean(sapply(sim_results, function(res) { if
(is.list(res\$bs)) { \# Check if true mean (-2 to 3 range) is in CI
res\$bs\$ci \<= 0.5 && 0.5 \<= res\$bs\$ci } else { FALSE } }))
...
{r} result \<- bs_mean(rnorm(50), R = 5000) \# Report: Mean = X (95% CI:
\[A, B\])
...
In your methods section: \> “We computed 95% confidence intervals using
the moving block bootstrap with block size = 15 to account for temporal
...
By default, `modernBoot` functions use sequential computation. For
faster results on large datasets:

{r, eval = FALSE} library(future)
...
❌ **Using standard bootstrap on dependent data** → Use block or
...
✅ **Do use
[`auto_select_method()`](https://github.com/ikrakib/modernBoot/reference/auto_select_method.md)
if unsure** → It helps guide your choice
...
{r, eval = FALSE} \# See all functions ?modernBoot

## Get help on specific function

?bs_mean ?wild_boot_lm ?perm_test_2sample
